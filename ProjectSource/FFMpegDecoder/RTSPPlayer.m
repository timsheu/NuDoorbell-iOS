...
- (BOOL)stepFrame
{
    ...
    while (!frameFinished && av_read_frame(pFormatCtx, &packet) >=0 ) {
        ...
        if(packet.stream_index==videoStream) {
            ...
            audioPacketQueueSize = audioPacketQueueSize + packet.size;
...
- (AVPacket*)readPacket
{
    ...
    NSMutableData *packetData = [NSMutableData dataWithData:[audioPacketQueue objectAtIndex:0]];
//    NSMutableData *packetData = [audioPacketQueue objectAtIndex:0];
    ...
    if (_packet) {
        ...
        [audioPacketQueueLock lock];
        audioPacketQueueSize = audioPacketQueueSize - _packet->size;
